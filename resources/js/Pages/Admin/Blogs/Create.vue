...
const submit = () => {
    ...
    if (imageFile.value) {
        ...
        router.post(route('admin.blogs.store'), fd, {
            onSuccess: () => {
                form.value = { name: '', slug: '', description: '', content: '', image: '', meta_title: '', meta_description: '', keywords: '', blog_category_id: '', status: true }
                clearFile()
            },
            onError: (errs) => {
                errors.value = errs
        ...
        router.post(route('admin.blogs.store'), form.value, {
            onSuccess: () => {
                form.value = { name: '', slug: '', description: '', content: '', image: '', meta_title: '', meta_description: '', keywords: '', blog_category_id: '', status: true, send_to_subscribers: false }
            },
            onError: (errs) => {
                errors.value = errs
                processing.value = false
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input v-model="form.slug" type="text"
                                       class="w-full rounded-sm border-gray-300 shadow-sm focus:border-[#2596be] focus:ring focus:ring-[#2596be]/20 text-sm" />
                                <p class="text-xs text-gray-400 mt-1">Auto-generated from title. Edit if needed.</p>
                            </div>
                            ...
                            <div>
                                ...
                                <input v-model="form.image" type="text" :disabled="!!imageFile"
                                       class="w-full rounded-sm border-gray-300 shadow-sm focus:border-[#2596be] focus:ring focus:ring-[#2596be]/20 text-sm" />
                                <p class="text-xs text-gray-400 mt-1">Paste a URL or upload a file below</p>
                            </div>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea v-model="form.content" rows="4"
                                          class="w-full rounded-sm border-gray-300 shadow-sm focus:border-[#2596be] focus:ring focus:ring-[#2596be]/20 text-sm"></textarea>
                                <p class="text-xs text-gray-400 mt-1">Short excerpt or plain text version</p>
                            </div>
                        </div>
                    </div>
